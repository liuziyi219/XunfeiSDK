...
int main(int argc, char* argv[])
{
	...
	const char* login_params			=	"appid = 5d162b2b, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	...
	*/
	...
	if (upload_on)
	{
        ...
        if((fpn=fopen("sortname.txt","r"))==NULL)
	{
         perror("fail to read filenames");
         exit(1);
        }
        while(fgets(buf,MAX_LINE,fpn)!=NULL)
        {
        len=strlen(buf);
        buf[len-1]='\0';
        char dir[]="./xwlb0302/";
	...
	run_iat(dir, session_begin_params); 
        }
exit:
	printf("按任意键退出 ...\n");
	getchar();
